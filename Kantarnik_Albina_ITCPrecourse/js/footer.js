...
} else if (codingLanguages.length === 1) {
    this_page.textContent = codingLanguages[0];
} else {
    this_page.textContent = codingLanguages.slice(0, codingLanguages.length - 1).join(", ") + " and " + codingLanguages[codingLanguages.length - 1];
}
